...
-- MACアドレス登録時に使用する登録順の番号。
-- 最大値を指定しないことで、設定可能な最大値が自動算出される。
...
ALTER SEQUENCE ACCOUNT_INFO_ACCOUNT_INFO_ID_SEQ INCREMENT 1 MINVALUE 1 RESTART 1;
...
(
	...
	-- 端末種別が3（VoIP-GW）の場合に使用する。
	-- ＜データ条件＞
	-- 端末種別が3（VoIP-GW）以外の場合はNULL。
...
-- 転送GW＋IP Voice(VPN接続)の接続先情報。
-- 上位SOからAPIによって通知される。
-- 本テーブルはユーザがVPNを契約中の場合に使用される。
CREATE TABLE EXTERNAL_GW_CONNECT_CHOICE_INFO
(
	-- 本テーブルのインデックスとして使用する一意なID。
	EXTERNAL_GW_CONNECT_CHOICE_INFO_ID SERIAL NOT NULL UNIQUE,
	-- どのN番の転送GW接続先候補情報かを示すN番情報ID。
	N_NUMBER_INFO_ID INT NOT NULL,
	-- APGWのグローバルIPアドレス。
	APGW_GLOBAL_IP INET NOT NULL,
	-- 転送GWのプライベートIP。
	-- AP-GWによってNATされたユーザ拠点内のIPアドレス。
	-- 外線番号の追加時に選択されたプライベートIPを保持する。
	-- ＜データ条件＞
	-- 外線サービス種別が6(転送GW＋IP Voice(VPN接続))の場合非NULL。それ以外の外線サービス種別の場合はNULL。
	EXTERNAL_GW_PRIVATE_IP INET NOT NULL,
	-- 最後に更新を実施したアカウントのアカウント情報ID。
	-- SOなど、カスコン画面からの更新でない場合は、0 を設定する。
	LAST_UPDATE_ACCOUNT_INFO_ID INT DEFAULT 0 NOT NULL,
	-- 最後に更新が実施された日時。
...
ALTER SEQUENCE MAC_ADDRESS_INFO_MAC_ADDRESS_INFO_ID_SEQ INCREMENT 1 MINVALUE 1 RESTART 1;
...
(
	...
	-- 外線サービス種別が 3（転送GW(C)＋ひかり電話/IP Voice（ひかり電話））,4（転送GW(i)＋ひかり電話/IP Voice（ひかり電話））,5（転送GW＋IP Voice(インターネット併用)）の場合、本システムの内線サーバが接続先となるSIPサーバに対してRegistする時に使用する番号。
	-- 接続先となるSIPサーバが払い出した内線番号。
	-- 
...
(
	...
	-- VMのインタネット向けのグローバルIPアドレス。
	-- 暗号化した状態で保持する。
	VM_GLOBAL_IP VARCHAR(256) NOT NULL,
	-- VMのインタネット向けフロント側（呼処理用）プライベートIPアドレス。
	VM_PRIVATE_IP_F INET NOT NULL,
	-- VMのインタネット向けバック側（保守作業用）プライベートIPアドレス。
	VM_PRIVATE_IP_B INET NOT NULL,
	-- VMのインタネット向けFQDN。
	-- 暗号化した状態で保持する。
	FQDN VARCHAR(512) NOT NULL,
	-- VMと対向しているAPGWのグローバルIPアドレス。
	-- 暗号化した状態で保持する。
	-- ＜データ条件＞
	-- VPN対応フラグ=falseの場合：NULL
	APGW_GLOBAL_IP VARCHAR(256),
	-- VMのVPN側に割り当てられるグローバルIPアドレス。
	-- ＜データ条件＞
	-- VPN対応フラグ=falseの場合：NULL
	VPN_GLOBAL_IP VARCHAR(256),
	-- VMのVPN側のNICに割り当てられるプライベートIPアドレス。
	-- ＜データ条件＞
	-- VPN対応フラグ=falseの場合：NULL
	VPN_PRIVATE_IP INET,
	-- VPNでAP-GWのユーザ拠点側に割り当てられる内線端末用IFのプライベートIPアドレス。
	-- ＜データ条件＞
	-- VPN対応フラグ=falseの場合：NULL
	VPN_FQDN_IP INET,
	-- VPNでAP-GWのユーザ拠点側に割り当てられるプライベートIPアドレスの第4オクテットの値。
	-- ＜データ条件＞
	-- VPN対応フラグ=falseの場合：NULL
	...
	-- VMのVPN対応/非対応を判別するフラグ。
	--  false:非対応
	--  true:対応
	VPN_USABLE_FLAG BOOLEAN DEFAULT 'false',
	-- VMの接続形態を示す値。
	-- NULL：インタネット接続のみ（既設）
	-- 　　0：インタネット接続のみ（未使用値）
	-- 　　1：VPN接続のみ
	...
	-- 　11：VPN移行待ち(元)
	-- 　　※SO変更後に、VPNの追加を受け付けた状態。
	-- 　12：VPN移行待ち(先)
	-- 　　※VPN移行先となっている状態。
	-- 　13：VPN移行予約済(元)
	-- 　　※VPN移転で、移転元として予約されている状態。
	-- 　14：VPN移行予約済(先)
	-- 　　※VPN移転で、移転先として予約されている状態。
	-- 　15：VPN移行中(元)
	-- 　　※VPN移転で、移転元として指定され、工事中の状態。
	-- 　16：VPN移行中(先)
	-- 　　※VPN移転で、移転先として指定され、工事中の状態。
...
ALTER SEQUENCE VM_INFO_VM_INFO_ID_SEQ INCREMENT 1 MINVALUE 1 RESTART 1;
...
-- ・追加後
-- CREATE UNIQUE INDEX UNIQUE_INDEX_LOGIN_ID ON ACCOUNT_INFO (LOGIN_ID) WHERE NOT delete_flag;
CREATE UNIQUE INDEX UIDX_VM_INFO_4 ON VM_INFO (VPN_GLOBAL_IP);
-- 一意な部分インデックス。
-- インデックス作成時には、以下のWHERE句を追加する。
-- 　WHERE NOT delete_flag
-- 【例】
-- ・追加前
CREATE UNIQUE INDEX UIDX_VM_INFO_5 ON VM_INFO (VPN_PRIVATE_IP);
...
端末種別が0（IP Phone）かつ、端末自動設定フラグがfalseの場合はNULL。
端末種別が0（IP Phone）かつ、端末自動設定フラグがtrueの場合は非NULL。';
COMMENT ON COLUMN EXTENSION_NUMBER_INFO.VPN_ACCESS_TYPE IS 'VoIP-GWの回線種別を示す値。
　0：Universal One
　1：IP-VPN
　2：e-VLAN
　3：Group-VPN
...
＜データ条件＞
端末種別が3（VoIP-GW）の場合はNULL。';
COMMENT ON COLUMN EXTENSION_NUMBER_INFO.VPN_LOCATION_N_NUMBER IS 'VPN拠点単位に付与されているVPN拠点N番情報。
端末種別が3（VoIP-GW）の場合に使用する。
＜データ条件＞
端末種別が3（VoIP-GW）の場合はNULL。';
...
　false：有効データ
　true：削除データ';
COMMENT ON TABLE EXTERNAL_GW_CONNECT_CHOICE_INFO IS '転送GW＋IP Voice(VPN接続)の接続先情報。
上位SOからAPIによって通知される。
本テーブルはユーザがVPNを契約中の場合に使用される。';
COMMENT ON COLUMN EXTERNAL_GW_CONNECT_CHOICE_INFO.EXTERNAL_GW_CONNECT_CHOICE_INFO_ID IS '本テーブルのインデックスとして使用する一意なID。';
COMMENT ON COLUMN EXTERNAL_GW_CONNECT_CHOICE_INFO.N_NUMBER_INFO_ID IS 'どのN番の転送GW接続先候補情報かを示すN番情報ID。';
COMMENT ON COLUMN EXTERNAL_GW_CONNECT_CHOICE_INFO.APGW_GLOBAL_IP IS 'APGWのグローバルIPアドレス。';
COMMENT ON COLUMN EXTERNAL_GW_CONNECT_CHOICE_INFO.EXTERNAL_GW_PRIVATE_IP IS '転送GWのプライベートIP。
AP-GWによってNATされたユーザ拠点内のIPアドレス。
外線番号の追加時に選択されたプライベートIPを保持する。
＜データ条件＞
外線サービス種別が6(転送GW＋IP Voice(VPN接続))の場合非NULL。それ以外の外線サービス種別の場合はNULL。';
COMMENT ON COLUMN EXTERNAL_GW_CONNECT_CHOICE_INFO.LAST_UPDATE_ACCOUNT_INFO_ID IS '最後に更新を実施したアカウントのアカウント情報ID。
SOなど、カスコン画面からの更新でない場合は、0 を設定する。';
COMMENT ON COLUMN EXTERNAL_GW_CONNECT_CHOICE_INFO.LAST_UPDATE_TIME IS '最後に更新が実施された日時。';
...
COMMENT ON COLUMN INCOMING_GROUP_INFO.GROUP_CALL_TYPE IS '着信グループの呼出方式。画面表示時は「グループ種別」。
　1：順次着信
　2：一斉着信
　3：コールピックアップ／コールパーク';
...
　false：チュートリアル未完了
　true ：チュートリアル完了済み';
COMMENT ON COLUMN N_NUMBER_INFO.MUSIC_HOLD_FLAG IS '保留音に、デフォルト音を使うか、カスコン画面から登録した音を使用するかを示すフラグ。
　false：デフォルトの保留音を利用する。
　true ：個別の保留音を利用する。';
...
COMMENT ON COLUMN OUTSIDE_CALL_INFO.SIP_CVT_REGIST_NUMBER IS '外線サービス種別が 3（転送GW(C)＋ひかり電話/IP Voice（ひかり電話））,4（転送GW(i)＋ひかり電話/IP Voice（ひかり電話））,5（転送GW＋IP Voice(インターネット併用)）の場合、本システムの内線サーバが接続先となるSIPサーバに対してRegistする時に使用する番号。
接続先となるSIPサーバが払い出した内線番号。
';
...
暗号化した状態で保持する。';
COMMENT ON COLUMN OUTSIDE_CALL_INFO.EXTERNAL_GW_PRIVATE_IP IS '転送GWのプライベートIP。
AP-GWによってNATされたユーザ拠点内のIPアドレス。
外線番号の追加時に選択されたプライベートIPを保持する。
＜データ条件＞
外線サービス種別が6(転送GW＋IP Voice(VPN接続))の場合非NULL。それ以外の外線サービス種別の場合はNULL。';
...
COMMENT ON COLUMN VM_INFO.VM_GLOBAL_IP IS 'VMのインタネット向けのグローバルIPアドレス。
暗号化した状態で保持する。';
COMMENT ON COLUMN VM_INFO.VM_PRIVATE_IP_F IS 'VMのインタネット向けフロント側（呼処理用）プライベートIPアドレス。';
COMMENT ON COLUMN VM_INFO.VM_PRIVATE_IP_B IS 'VMのインタネット向けバック側（保守作業用）プライベートIPアドレス。';
COMMENT ON COLUMN VM_INFO.FQDN IS 'VMのインタネット向けFQDN。
暗号化した状態で保持する。';
COMMENT ON COLUMN VM_INFO.APGW_GLOBAL_IP IS 'VMと対向しているAPGWのグローバルIPアドレス。
暗号化した状態で保持する。
＜データ条件＞
VPN対応フラグ=falseの場合：NULL';
COMMENT ON COLUMN VM_INFO.VPN_GLOBAL_IP IS 'VMのVPN側に割り当てられるグローバルIPアドレス。
＜データ条件＞
VPN対応フラグ=falseの場合：NULL';
COMMENT ON COLUMN VM_INFO.VPN_PRIVATE_IP IS 'VMのVPN側のNICに割り当てられるプライベートIPアドレス。
＜データ条件＞
VPN対応フラグ=falseの場合：NULL';
COMMENT ON COLUMN VM_INFO.VPN_FQDN_IP IS 'VPNでAP-GWのユーザ拠点側に割り当てられる内線端末用IFのプライベートIPアドレス。
＜データ条件＞
VPN対応フラグ=falseの場合：NULL';
COMMENT ON COLUMN VM_INFO.VPN_FQDN_OCTET_FOUR IS 'VPNでAP-GWのユーザ拠点側に割り当てられるプライベートIPアドレスの第4オクテットの値。
＜データ条件＞
VPN対応フラグ=falseの場合：NULL';
...
';
COMMENT ON COLUMN VM_INFO.VPN_N_NUMBER IS 'VPNの代表N番。
＜データ条件＞
VPN対応フラグ=falseの場合：NULL';
COMMENT ON COLUMN VM_INFO.APGW_FUNCTION_NUMBER IS 'APGWの利用機能番号。
＜データ条件＞
VPN対応フラグ=falseの場合：NULL';
COMMENT ON COLUMN VM_INFO.BHEC_N_NUMBER IS '内線サーバのVMが属するBHECのN番。';
COMMENT ON COLUMN VM_INFO.APGW_N_NUMBER IS 'APGWを識別するN番。
＜データ条件＞
VPN対応フラグ=falseの場合：NULL';
COMMENT ON COLUMN VM_INFO.VPN_USABLE_FLAG IS 'VMのVPN対応/非対応を判別するフラグ。
 false:非対応
 true:対応';
